use tyl_errors::{TylError, TylResult, ErrorClassifier, ErrorCategory};
...
fn custom_categories_example() {
    ...
    let payment_error = TylError::business_logic(
        ...
        match process_payment(amount) {
            ...
            Err(error) => {
                ...
                println!(
                    ...
                    error,
                    category.category_name(),
                    if category.is_retriable() { " - Can retry" } else { " - Permanent" }
                );
